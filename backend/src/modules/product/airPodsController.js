...
import AirPods, { AirPodsVariant } from "./AirPods.js";
...
export const create = async (req, res) => {
  ...
  try {
    console.log("CREATE AIRPODS REQUEST:", JSON.stringify(req.body, null, 2));
    ...
    const existingBySlug = await AirPods.findOne({
    ...
    if (variantGroups.length > 0) {
      ...
      for (const group of variantGroups) {
        ...
        for (const opt of options) {
          ...
          // const existingVariantSlug = await AirPodsVariant.findOne({ slug: variantSlug }).session(session);
          // if (existingVariantSlug) throw new Error(`Variant slug đã tồn tại: ${variantSlug}`);

          const variantDoc = new AirPodsVariant({
    ...
    const populated = await AirPods.findById(product._id)
      .populate("variants")
    ...
    console.error("CREATE AIRPODS ERROR:", error.message);
...
export const update = async (req, res) => {
  ...
  try {
    ...
    console.log("UPDATE AIRPODS REQUEST:", id);

    const product = await AirPods.findById(id).session(session);
    ...
    if (newSlug !== (product.slug || product.baseSlug)) {
      const slugExists = await AirPods.findOne({
    ...
    if (variantGroups.length > 0) {
      ...
      await AirPodsVariant.deleteMany({ productId: id }, { session });
      const newIds = [];
    ...
    const populated = await AirPods.findById(id)
      .populate("variants")
      .populate("createdBy", "fullName email");
    ...
    await session.abortTransaction();
    console.error("UPDATE AIRPODS ERROR:", error);
    res.status(400).json({
      success: false,
...
export const getProductDetail = async (req, res) => {
  try {
    ...
    console.log("getProductDetail AirPods:", { slug, sku: skuQuery });

    let variant = await AirPodsVariant.findOne({ slug });
    let product = null;

    if (variant) {
      product = await AirPods.findById(variant.productId)
        .populate("variants")
        .populate("createdBy", "fullName email");
    ...
    console.error("getProductDetail AirPods error:", error);
    res.status(500).json({
      success: false,
...
export const deleteAirPods = async (req, res) => {
  ...
  try {
    const product = await AirPods.findById(req.params.id).session(session);
    if (!product) throw new Error("Không tìm thấy sản phẩm");

    await AirPodsVariant.deleteMany({ productId: product._id }, { session });
