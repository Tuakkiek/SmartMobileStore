...
import Accessory, { AccessoryVariant } from "./Accessory.js";
...
export const create = async (req, res) => {
  ...
  try {
    console.log("CREATE ACCESSORY REQUEST:", JSON.stringify(req.body, null, 2));
    ...
    if (variantGroups.length > 0) {
      ...
      for (const group of variantGroups) {
        ...
        for (const opt of options) {
          ...
          // const existingVariantSlug = await AccessoryVariant.findOne({ slug: variantSlug }).session(session);
    ...
    console.error("CREATE ACCESSORY ERROR:", error.message);
...
export const update = async (req, res) => {
  ...
  try {
    ...
    console.log("UPDATE ACCESSORY REQUEST:", id);
    ...
    if (data.specifications) {
      ...
      product.specifications = {
        ...data.specifications,
        colors,
        customSpecs: data.specifications.customSpecs || product.specifications.customSpecs || [],
      };
    }
...
export const getProductDetail = async (req, res) => {
  try {
    ...
    console.log("getProductDetail Accessory:", { slug, sku: skuQuery });
    ...
    console.error("getProductDetail Accessory error:", error);
    res.status(500).json({
      success: false,
