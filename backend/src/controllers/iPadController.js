...
// Variant slug: baseSlug-color-storage-connectivity
const createVariantSlug = (baseSlug, color, storage, connectivity) => {
  ...
  return `${baseSlug}-${colorSlug}-${storageSlug}-${connectivitySlug}`;
};
    ...
    if (variantGroups.length > 0) {
      for (const group of variantGroups) {
        ...
        for (const opt of options) {
          ...
          const existingVariantSlug = await IPadVariant.findOne({
            slug: variantSlug,
          }).session(session);
