...
export const create = async (req, res) => {
  ...
  try {
    ...
    if (variantsToCreate.length > 0) {
      ...
      const allVariantNames = variantsToCreate
        .flatMap((v) => v.options.map((o) => o.variantName.trim()))
        .filter(Boolean);
      const uniqueVariantNames = [...new Set(allVariantNames)];

      product.specifications.colors = allColors;
      product.specifications.variantName = uniqueVariantNames.join(" / ");
    ...
    const populatedProduct = await AirPods.findById(product._id)
      .populate("variants")
...
export const update = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      // Delete old variants
      const deleteResult = await AirPodsVariant.deleteMany(
        { productId: product._id },
        { session }
      );
      ...
      const allVariantNames = variantsToUpdate
        .flatMap((v) => v.options.map((o) => o.variantName.trim()))
        .filter(Boolean);
      const uniqueVariantNames = [...new Set(allVariantNames)];
